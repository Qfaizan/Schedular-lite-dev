...
const App = () => {
  ...
  const handleModal = (type: string) => {
    ...
    if (type === 'address') {
      dispacth(
        updateForm({ isAddress: true, govtIdFrontCard: '', govtIdBackCard: '' })
      );
  ...
  const handleSubmitForm = () => {
    if (!validateForm(form)) dispacth(setSubmit(true));
  ...
  return (
    <div style={{ width: '100%' }}>
      {form.location !== null &&
      form?.location?.schedularLite?.isActive &&
      !(appointment?.ack !== 'error' && appointment?.ack !== '') ? (
        <>
          ...
          <div className="flex w-full justify-center">
            <div className="w-10/12">
              ...
              <TestTypes
                testTypeItems={form.location.schedularLite.testTypes}
              />
              ...
              <Button
                ...
                text={
                  appointment.process ? 'Processing... ' : 'Book Appointment'
                }
                onClick={handleSubmitForm}
                className="max-w-min btn my-6 font-bold shadow-lg fill-btn"
              />
              {appointment.ack === 'error' && (
                <p className="text-red-600 mb-5 text-lg text-center">
          ...
          {alertBox?.open && (
            <Dialog
              open={alertBox?.open}
              onClose={() =>
                dispacth(handleDilogModal({ open: false, title: '', body: '' }))
              }
            >
              <DialogTitle className="font-extrabold f-9">
                {alertBox?.title}
              </DialogTitle>
              <DialogContent>
                <ul>
                  {alertBox?.body?.split(', ').map((e: any, index: number) => (
                    <li className={index === 0 ? `font-semibold mb-3` : ``}>
                      {e}
                    </li>
                  ))}
                </ul>
          ...
          <div className="flex w-screen h-full justify-center items-center relative">
            {form.location === null ? (
              ...
              <>
                ...
                {appointment?.ack ? (
                  <div className="bg-slate-200 shadow-lg p-10 rounded-md">
                    <p className="text-3xl mb-5">
                      Your Appointment Successfully Placed
                    </p>
                    <p className="text-3xl font-semibold uppercase underline mt-5">
                      {appointment?.conf
                        ? '# Confirmation ID'
                        : '# Acknowledgement ID'}
                    </p>
                    <p className="text-2xl font-bold mt-5">
                      {appointment.conf ? appointment.conf : appointment.ack}
                    </p>
                  </div>
                ) : (
