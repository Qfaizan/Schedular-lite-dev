        ...
        <div className="my-4 w-full">
				...
				{Contacts.map((e, i) => !(count>1 && e.name==='email')? (
					<div className="my-5" key={i}>
                        {console.log('KC test',e.name,isErrorField(submit,e.name, form,error))}
						<div className="w-full flex justify-around gap-2 items-center">
                            ...
                            (
                                <Button 
                                    ...
                                    className={`btn ${InputBoxSize==='small'?'':'h-55'} elevated-btn`}
                                />
							)}
							{((form?.isPhnVerified && e.name === "phone") ||
								(form?.isEmailVerified && e.name === "email")) && (
								<MdOutlineVerified className={`text-${InputBoxSize==='small'?'5':'7'}xl text-green`} />
							)}
						</div>
