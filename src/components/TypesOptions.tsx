    ...
    return (
      <>
        <div className="flex flex-wrap gap-4 items-center w-60 lg:w-full md:w-60">
          ...
          {ENTER_ADDRESS_MANUALLY && PROVIDE_GOVT_PROOF && (
            <h4 className="">Choose which one to add:</h4>
          )}
          <div
            style={{ width: 500 }}
            className={`relative w-full flex items-center gap-4 flex-wrap xl:w-2/12 lg:w-3/12 md:w-full sm:w-full ${
          ...
          >
            ...
            {ENTER_ADDRESS_MANUALLY && (
              <div className="flex items-center h-full xl:pl-8">
                <Button
                  text={`${form.isAddress ? 'Edit Address' : 'Add Address'}`}
                  type="button"
                  disabled={form?.isGovtProof}
                  icon={{
                    position: 'left',
                    icon: form.isAddress ? <BiEditAlt /> : <BsPlusLg />,
                  }}
                  onClick={() =>
                    dispacth(
                      setModal({
                  ...
                  className={`font-bold p-4 rounded-full flex justify-center h-10 items-center btn ${
                    form?.isAddress ? 'text-btn' : 'elevated-btn'
                  }`}
                />
              </div>
            )}
            ...
            {PROVIDE_GOVT_PROOF && (
              <div className="xl:pl-8">
                <Button
                  ...
                  icon={{
                    position: 'left',
                    icon: form.isGovtProof ? <BiEditAlt /> : <BsPlusLg />,
                  }}
                  onClick={() =>
                    dispacth(
                      setModal({
                  ...
                  className={`font-bold p-4  rounded-full flex justify-center h-10 items-center btn ${
                    form?.isGovtProof ? 'text-btn' : 'elevated-btn'
                  }`}
                />
              </div>
            )}
    ...
    if (ENTER_INSURANCE_DETAILS || CREDIT_CARD || ALLOW_NO_INSURANCE)
      return (
        <>
          ...
          {ENTER_INSURANCE_DETAILS && CREDIT_CARD && (
            <h4 className="my-3">Choose how to pay:</h4>
          )}
          <div
            className={`relative w-full p-2 xl:w-2/12 lg:w-3/12 md:w-full sm:w-full ${
                ...
                <Button
                  ...
                  icon={{
                    ...
                    icon:
                      form.payingMethod === 'Credit Card' &&
                      !form.iDontHaveInsurance ? (
                        <GrCheckboxSelected className="bg-white" />
                      ) : (
                        <BsPlusLg />
                      ),
                  }}
                  ...
                  className={`font-bold p-4 mb-4 rounded-full flex justify-center h-10 items-center btn ${
                    form.payingMethod === 'Credit Card' &&
                    !form.iDontHaveInsurance
                      ? 'bg-dark text-white'
                      : 'elevated-btn'
